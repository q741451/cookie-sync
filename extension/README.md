...
2. 填写服务器地址（你的后端根地址，比如 `https://your-domain.com`）
3. 建立/加入频道（可以有多个）：
   - **新建频道**：填一个频道名，点击「创建」，系统会自动生成密钥并保存
   - **加入已有频道**：把小伙伴给你的频道名 + 密钥填进「加入已有频道」，点击「加入」
   - 频道列表里第一个添加的频道会自动成为「默认」，也可以随时点其他频道的单选框切换默认
4. （可选）需要某些网站走特定频道时，在「网站规则」里加一条，比如域名填 `jd.com`、
   选择对应频道 —— 这样 `order.jd.com`、`www.jd.com` 等所有 `jd.com` 的子域名都会自动
   走这个频道，不用逐个网站配置。没配规则的网站统一走默认频道。
5. 回到任意网站，点击插件图标，会显示当前网站将使用哪个频道（默认还是按规则匹配）：
